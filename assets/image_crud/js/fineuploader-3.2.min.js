...
qq.log = function(a, b) {
    if (window.console)
        if (!b || "info" === b) window.console.log(a);
        else if (window.console[b]) window.console[b](a);
...
qq.each = function(a, b) {
    ...
    if (a)
        for (c in a)
            if (Object.prototype.hasOwnProperty.call(a, c) && (d = b(c, a[c]), !1 === d)) break
};
qq.obj2url = function(a, b, c) {
    ...
    if (!c && b) f = /\?/.test(b) ? /\?$/.test(b) ? "" : "&" : "?", e.push(b), e.push(qq.obj2url(a));
        ...
        for (d in a) a.hasOwnProperty(d) && g(a[d], d);
    else e.push(encodeURIComponent(b) + "=" + encodeURIComponent(a));
...
qq.obj2FormData = function(a, b, c) {
    ...
    qq.each(a, function(a, e) {
        a = c ? c + "[" + a + "]" : a;
        qq.isObject(e) ? qq.obj2FormData(e, b, a) : qq.isFunction(e) ? b.append(encodeURIComponent(a), encodeURIComponent(e())) : b.append(encodeURIComponent(a), encodeURIComponent(e))
    });
    return b
};
...
qq.parseJson = function(a) {
    return "function" === typeof JSON.parse ? JSON.parse(a) : eval("(" + a + ")")
};
...
qq.UploadButton.prototype = {
    ...
    _createInput: function() {
        ...
        this._options.acceptFiles && a.setAttribute("accept", this._options.acceptFiles);
        ...
        var b = this;
        this._disposeSupport.attach(a, "change", function() {
...
qq.FineUploaderBasic.prototype = {
    ...
    uploadStoredFiles: function() {
        for (var a; this._storedFileIds.length;) a = this._storedFileIds.shift(),
            this._filesInProgress.push(a), this._handler.upload(a)
    },
    ...
    addFiles: function(a) {
        ...
        if (a) {
            ...
            for (c = 0; c < a.length; c += 1) d = a[c], qq.isFileOrInput(d) ? b.push(d) : this.log(d + " is not a File or INPUT element!  Ignoring!", "warn");
            this.log("Processing " + b.length + " files or inputs...");
            this._uploadFileList(b)
        }
    },
    getUuid: function(a) {
    ...
    _createUploadHandler: function() {
        ...
        return new qq.UploadHandler({
            ...
            onComplete: function(b, c, d, e) {
                ...
                a._options.callbacks.onComplete(b, c, d)
            },
            ...
            onAutoRetry: function(b, c, d, e) {
                a._preventRetries[b] =
                    d[a._options.retry.preventRetryResponseProperty];
    ...
    _onCancel: function(a) {
        ...
        a = qq.indexOf(this._storedFileIds, a);
        !this._options.autoUpload && 0 <= a && this._storedFileIds.splice(a, 1)
    },
    _removeFromFilesInProgress: function(a) {
        a = qq.indexOf(this._filesInProgress, a);
        0 <= a && this._filesInProgress.splice(a,
            1)
    },
    _onUpload: function() {},
    _onInputChange: function(a) {
        qq.isXhrUploadSupported() ? this.addFiles(a.files) : this.addFiles(a);
        this._button.reset()
    },
    ...
    _onBeforeManualRetry: function(a) {
        ...
        if (this._handler.isValid(a)) {
            ...
            if (!1 === this._options.callbacks.onManualRetry(a, b)) return !1;
            this.log("Retrying upload for '" + b + "' (id: " + a + ")...");
    ...
    _maybeParseAndSendUploadError: function(a,
        b, c, d) {
        if (!c.success)
            if (d && 200 !== d.status && !c.error) this._options.callbacks.onError(a, b, "XHR returned response code " + d.status);
            else this._options.callbacks.onError(a, b, c.error ? c.error : "Upload failure reason unknown")
    },
    _uploadFileList: function(a) {
        var b;
        if (!1 !== this._options.callbacks.onValidateBatch(this._getValidationDescriptors(a)))
            if (0 < a.length)
                for (b = 0; b < a.length; b++)
                    if (this._validateFile(a[b])) this._uploadFile(a[b]);
                    else {
    ...
    _uploadFile: function(a) {
        ...
        !1 !== this._options.callbacks.onSubmit(a, b) && (this._onSubmit(a, b), this._options.autoUpload ? this._handler.upload(a) : this._storeFileForLater(a))
    },
    _storeFileForLater: function(a) {
        this._storedFileIds.push(a)
    },
    _validateFile: function(a) {
        var b, c, a = this._getValidationDescriptor(a);
        b = a.name;
        c = a.size;
        if (!1 === this._options.callbacks.onValidate(a)) return !1;
        if (this._isAllowedExtension(b)) {
            if (0 === c) return this._error("emptyError",
                b), !1;
            if (c && this._options.validation.sizeLimit && c > this._options.validation.sizeLimit) return this._error("sizeError", b), !1;
            if (c && c < this._options.validation.minSizeLimit) return this._error("minSizeError", b), !1
        } else return this._error("typeError", b), !1;
        return !0
    },
    _error: function(a, b) {
        var c = this._options.messages[a],
            d = this._options.validation.allowedExtensions.join(", ").toLowerCase(),
            e = this._options.formatFileName(b),
            c = c.replace("{file}", e),
            c = c.replace("{extensions}", d),
            d = this._formatSize(this._options.validation.sizeLimit),
            c = c.replace("{sizeLimit}", d),
            d = this._formatSize(this._options.validation.minSizeLimit),
            c = c.replace("{minSizeLimit}", d);
        this._options.callbacks.onError(null, b, c);
        return c
    },
    _isAllowedExtension: function(a) {
        ...
        qq.each(b, function(b, e) {
            if (null != a.match(RegExp("\\." + e + "$", "i"))) return c = !0, !1
        });
        return c
    },
    ...
    _parseFileName: function(a) {
        return a.value ? a.value.replace(/.*(\/|\\)/, "") : null !== a.fileName && void 0 !== a.fileName ? a.fileName : a.name
    },
    _parseFileSize: function(a) {
        var b;
        a.value || (b = null !== a.fileSize &&
            void 0 !== a.fileSize ? a.fileSize : a.size);
        return b
    },
    _getValidationDescriptor: function(a) {
        var b, c;
        c = {};
        b = this._parseFileName(a);
        a = this._parseFileSize(a);
        c.name = b;
        a && (c.size = a);
        return c
    },
    ...
    _createParamsStore: function() {
        ...
        return {
            ...
            getParams: function(c) {
                var d = {};
                null != c && a[c] ? qq.extend(d, a[c]) : qq.extend(d, b._options.request.params);
                return d
            },
            remove: function(b) {
                return delete a[b]
...
qq.DragAndDrop = function(a) {
    function b() {
        p === n && !k && (g.callbacks.log("Grabbed " + l.length + " files after tree traversal."), h.dropDisabled(!1), g.callbacks.dropProcessing(!1, l))
    }
    ...
    function c(a) {
        ...
        a.isFile ? a.file(function(a) {
            ...
            n += 1;
            b()
        }) : a.isDirectory && (k = !0, a = a.createReader(), a.readEntries(function(a) {
            n += 1;
            for (d = 0; d < a.length; d += 1) c(a[d]);
            k = !1;
            a.length || b()
        }))
    }
    ...
    function d(a) {
        h = new qq.UploadDropZone({
            element: a,
            onEnter: function(b) {
                qq(a).addClass(g.classes.dropActive);
                b.stopPropagation()
            },
            onLeaveNotDescendants: function() {
                qq(a).removeClass(g.classes.dropActive)
            },
            onDrop: function(d) {
                g.hideDropzones && qq(a).hide();
                qq(a).removeClass(g.classes.dropActive);
                var d = d.dataTransfer,
                    e, f;
                g.callbacks.dropProcessing(!0);
                h.dropDisabled(!0);
                if (1 < d.files.length && !g.multiple) g.callbacks.dropProcessing(!1), g.callbacks.error("tooManyFilesError", ""), h.dropDisabled(!1);
                else if (l = [], n = p = 0, qq.isFolderDropSupported(d)) {
                    e = d.items;
                    for (d = 0; d < e.length; d += 1)
                        if (f = e[d].webkitGetAsEntry()) f.isFile ? (l.push(e[d].getAsFile()), d === e.length - 1 && b()) : c(f)
                } else g.callbacks.dropProcessing(!1, d.files),
                    h.dropDisabled(!1)
            }
        });
    ...
    function f() {
        g.dropArea && g.extraDropzones.push(g.dropArea);
        var a, b = g.extraDropzones;
        for (a = 0; a < b.length; a += 1) d(b[a]);
        g.dropArea && (!qq.ie() || qq.ie10()) && q.attach(document, "dragenter", function(c) {
            if (!h.dropDisabled() && e(c) && !qq(g.dropArea).hasClass(g.classes.dropDisabled)) {
                g.dropArea.style.display = "block";
                for (a =
                    0; a < b.length; a += 1) b[a].style.display = "block"
            }
        });
        q.attach(document, "dragleave", function(c) {
            if (g.hideDropzones && qq.FineUploader.prototype._leaving_document_out(c))
                for (a = 0; a < b.length; a += 1) qq(b[a]).hide()
        });
        q.attach(document, "drop", function(c) {
            if (g.hideDropzones)
                for (a = 0; a < b.length; a += 1) qq(b[a]).hide();
            c.preventDefault()
        })
    ...
    return {
        ...
        setupExtraDropzone: function(a) {
            g.extraDropzones.push(a);
            d(a)
        },
        removeExtraDropzone: function(a) {
            var b, c = g.extraDropzones;
            for (b in c)
...
qq.UploadDropZone = function(a) {
    ...
    var e, f, g, h, k = new qq.DisposeSupport;
    e = {
        element: null,
        onEnter: function() {},
        onLeave: function() {},
    ...
    f = e.element;
    (function() {
        h || (b ? k.attach(document, "dragover", function(a) {
            a.preventDefault()
    ...
    (function() {
        k.attach(f, "dragover", function(a) {
            if (c(a)) {
        ...
        k.attach(f, "dragenter", function(a) {
            if (!d() && c(a)) e.onEnter(a)
        });
        k.attach(f, "dragleave",
            function(a) {
                if (c(a)) {
                    e.onLeave(a);
...
qq.FineUploader = function(a) {
    ...
    qq.extend(this._options, a, !0);
    this._wrapCallbacks();
    this._options.template = this._options.template.replace(/\{dragZoneText\}/g,
        this._options.text.dragZone);
    this._options.template = this._options.template.replace(/\{uploadButtonText\}/g, this._options.text.uploadButton);
    this._options.template = this._options.template.replace(/\{dropProcessingText\}/g, this._options.text.dropProcessing);
    this._options.fileTemplate = this._options.fileTemplate.replace(/\{cancelButtonText\}/g, this._options.text.cancelButton);
    this._options.fileTemplate = this._options.fileTemplate.replace(/\{retryButtonText\}/g, this._options.text.retryButton);
    this._options.fileTemplate =
        this._options.fileTemplate.replace(/\{statusText\}/g, "");
    this._element = this._options.element;
...
qq.extend(qq.FineUploader.prototype, {
    ...
    _setupDragAndDrop: function() {
        ...
        c = new qq.DragAndDrop({
            ...
            callbacks: {
                dropProcessing: function(c, f) {
                    var g = a._button.getInput();
                    c ? (qq(b).css({
                        display: "block"
                    }), qq(g).attach("click", d)) : (qq(b).hide(), qq(g).detach("click",
                        d));
                    f && a.addFiles(f)
                },
                error: function(b, c) {
    ...
    _find: function(a, b) {
        ...
        if (!c) throw Error("element not found " +
            b);
        return c
    },
    ...
    _onProgress: function(a, b, c, d) {
        ...
        h = Math.round(100 * (c / d));
        c === d ? (g = this._find(e, "cancel"), qq(g).hide(), qq(f).hide(), qq(this._find(e, "statusText")).setText(this._options.text.waitingForResponse), g = this._formatSize(d)) : (g = this._formatProgress(c, d), qq(f).css({
            display: "block"
        }));
        qq(f).css({
            width: h + "%"
    ...
    _onComplete: function(a, b, c, d) {
        ...
        (!this._options.disableCancelForFormUploads || qq.isXhrUploadSupported()) && qq(this._find(e, "cancel")).hide();
        qq(this._find(e, "spinner")).hide();
        c.success ? (qq(e).addClass(this._classes.success),
    ...
    _onUpload: function(a, b) {
        ...
        this._showSpinner(this.getItemByFileId(a))
    },
    _onBeforeAutoRetry: function(a) {
        ...
        c.style.width = 0;
        qq(c).hide();
        this._options.retry.showAutoRetryNote && (c = this._find(b, "statusText"), d = this._autoRetries[a] + 1, e = this._options.retry.maxAutoAttempts, f = this._options.retry.autoRetryNote.replace(/\{retryNum\}/g, d), f = f.replace(/\{maxAuto\}/g, e), qq(c).setText(f),
            1 === d && qq(b).addClass(this._classes.retrying))
    },
    _onBeforeManualRetry: function(a) {
        if (qq.FineUploaderBasic.prototype._onBeforeManualRetry.apply(this, arguments)) {
            var b = this.getItemByFileId(a);
            this._find(b, "progressBar").style.width = 0;
            qq(b).removeClass(this._classes.fail);
            qq(this._find(b, "statusText")).clearText();
            this._showSpinner(b);
            this._showCancelLink(b);
            return !0
        }
    ...
    _bindCancelAndRetryEvents: function() {
        ...
        this._disposeSupport.attach(this._listElement, "click", function(b) {
            ...
            if (qq(c).hasClass(a._classes.cancel) ||
                ...
                qq.preventDefault(b);
                for (b = c.parentNode; void 0 == b.qqFileId;) b = c = c.parentNode;
                qq(c).hasClass(a._classes.cancel) ? a.cancel(b.qqFileId) : (qq(b).removeClass(a._classes.retryable), a.retry(b.qqFileId))
            }
        })
    },
    _formatProgress: function(a, b) {
        var c = this._options.text.formatProgress,
            ...
            c = c.replace("{percent}", d),
            d = this._formatSize(b);
    ...
    _showCancelLink: function(a) {
        if (!this._options.disableCancelForFormUploads || qq.isXhrUploadSupported()) this._find(a, "cancel").style.display = "inline"
    },
    _error: function(a, b) {
        this._options.showMessage(qq.FineUploaderBasic.prototype._error.apply(this, arguments))
    }
});
qq.UploadHandler = function(a) {
    ...
    d = c.log;
    e = function(a) {
        var a = qq.indexOf(b, a),
            d = c.maxConnections;
        b.splice(a, 1);
        b.length >= d && a < d && (a = b[d - 1], f.upload(a))
    };
    f = qq.isXhrUploadSupported() ? new qq.UploadHandlerXhr(c, e, d) : new qq.UploadHandlerForm(c, e, d);
    return {
        add: function(a) {
            return f.add(a)
        },
        ...
        retry: function(a) {
            return 0 <=
                qq.indexOf(b, a) ? f.upload(a, !0) : this.upload(a)
        },
        cancel: function(a) {
            d("Cancelling " + a);
...
qq.UploadHandlerForm = function(a, b, c) {
    function d(a, b) {
        p[a.id] = qq(a).attach("load", function() {
    ...
    function e(a) {
        ...
        try {
            ...
            n("innerHTML = " +
                d);
            d && d.match(/^<pre/i) && (d = c.body.firstChild.firstChild.nodeValue);
            b = eval("(" + d + ")")
        } catch (e) {
            n("Error when attempting to parse form upload response (" + e + ")", "error"), b = {
                success: !1
            }
        }
        return b
    ...
    function g(a, b) {
        var c = h.paramsStore.getParams(a),
            d = qq.toElement('<form method="' + (h.demoMode ? "GET" : "POST") + '" enctype="multipart/form-data"></form>'),
            e = h.endpointStore.getEndpoint(a),
            f = e;
        c[h.uuidParamName] = l[a];
        h.paramsInBody ? qq.obj2Inputs(c, d) : f = qq.obj2url(c, e);
    ...
    return q = {
        add: function(a) {
            a.setAttribute("name", h.inputName);
            var b = k.push(a) - 1;
            l[b] = qq.getUniqueId();
        ...
        upload: function(a) {
            ...
            d(s, function() {
                ...
                if (c.success || !h.onAutoRetry(a, l, c)) h.onComplete(a, l, c), b(a)
            });
...
qq.UploadHandlerXhr = function(a, b, c) {
    function d(a, b, c) {
        var d = m.getSize(a),
            a = m.getName(a);
        b[i.chunking.paramNames.partIndex] = c.part;
        b[i.chunking.paramNames.partByteOffset] = c.start;
        b[i.chunking.paramNames.chunkSize] = c.end - c.start;
        b[i.chunking.paramNames.totalParts] = c.count;
        b[i.totalFileSizeParamName] = d;
        v && (b[i.chunking.paramNames.filename] = a)
    }

    function e(a, b) {
        var c = i.chunking.partSize,
            d = m.getSize(a),
            e = j[a].file,
            g = c * b,
            c = g + c >= d ? d : g + c,
            d = f(a),
            h;
        e.slice ? h = e.slice(g, c) : e.mozSlice ? h = e.mozSlice(g, c) : e.webkitSlice &&
            (h = e.webkitSlice(g, c));
        return {
            part: b,
            start: g,
    ...
    function f(a) {
        a = m.getSize(a);
        return Math.ceil(a / i.chunking.partSize)
    }

    function g(a) {
        j[a].xhr = new XMLHttpRequest;
        return j[a].xhr
    }

    function h(a, b, c, d) {
        var e = new FormData,
            f = i.demoMode ? "GET" : "POST",
            g = i.endpointStore.getEndpoint(d),
            h = g,
            k = m.getName(d),
            l = m.getSize(d);
        a[i.uuidParamName] = j[d].uuid;
        v && (a[i.totalFileSizeParamName] = l);
        i.paramsInBody || (a[i.inputName] = k, h = qq.obj2url(a, g));
        b.open(f, h, !0);
        return v ? (i.paramsInBody && qq.obj2FormData(a,
            e), e.append(i.inputName, c), e) : c
    }

    function k(a, b) {
        var c = i.customHeaders;
        m.getName(a);
        var d = j[a].file;
        b.setRequestHeader("X-Requested-With", "XMLHttpRequest");
        b.setRequestHeader("Cache-Control", "no-cache");
        v || (b.setRequestHeader("Content-Type", "application/octet-stream"), b.setRequestHeader("X-Mime-Type", d.type));
        qq.each(c, function(a, c) {
    ...
    function l(a, b, c) {
        var d = m.getName(a),
            e = m.getSize(a);
        j[a].attemptingResume = !1;
        i.onProgress(a, d, e, e);
        i.onComplete(a, d, b, c);
        delete j[a].xhr;
        C(a)
    }

    function p(a) {
        var b = e(a, j[a].remainingChunkIdxs[0]),
            c = g(a),
            f = m.getSize(a),
            A = m.getName(a),
            t;
        void 0 === j[a].loaded && (j[a].loaded = 0);
        B(a, b);
        c.onreadystatechange = q(a, c);
        c.upload.onprogress = function(b) {
            if (b.lengthComputable && j[a].loaded < f) i.onProgress(a, A, b.loaded + j[a].loaded, f)
        };
        i.onUploadChunk(a, A, n(b));
        t = i.paramsStore.getParams(a);
        d(a, t, b);
        j[a].attemptingResume && (t[i.resume.paramNames.resuming] = !0);
        t = h(t, c, b.blob, a);
        k(a, c);
        r("Sending chunked upload request for " + a + ": bytes " + (b.start + 1) + "-" + b.end + " of " +
            f);
        c.send(t)
    }
            ...
            if (4 === b.readyState) {
                ...
                if (j[a]) {
                    ...
                    try {
                        ...
                        r("Error when attempting to parse xhr response text (" + d + ")", "error"), c = {}
                    }
                    if (200 !== b.status || !c.success || c.reset)
                        if (c.reset && (r("Server has ordered chunking effort to be restarted on next attempt for file ID " +
                                a, "error"), w && y(a), j[a].remainingChunkIdxs = [], delete j[a].loaded), j[a].attemptingResume && c.reset) j[a].attemptingResume = !1, r("Server has declared that it cannot handle resume for file ID " + a + " - starting from the first chunk", "error"), m.upload(a, !0);
                        else {
                            var f = m.getName(a);
                            i.onAutoRetry(a, f, c, b) || l(a, c, b)
                        }
                    else x ? (f = j[a].remainingChunkIdxs.shift(), f = e(a, f), j[a].attemptingResume = !1, j[a].loaded += f.end - f.start, 0 < j[a].remainingChunkIdxs.length ? p(a) : (y(a), l(a, c, b))) : l(a, c, b)
                }
            }
        }
    }
    ...
    function B(a, b) {
        var c = m.getUuid(a),
            d = z(a);
        qq.setCookie(d, c + o + b.part, i.resume.cookiesExpireIn)
    }
    ...
    function z(a) {
        var b = m.getName(a),
            a = m.getSize(a),
            c = i.chunking.partSize,
            b = "qqfilechunk" + o + encodeURIComponent(b) + o + a + o + c;
        void 0 !== u && (b += o + u);
        return b
    }
    ...
    function s(a) {
        ...
        d.upload.onprogress = function(b) {
            b.lengthComputable && (j[a].loaded = b.loaded, i.onProgress(a, c, b.loaded, b.total))
        };
        d.onreadystatechange = q(a, d);
        e = i.paramsStore.getParams(a);
        b = h(e, d, b, a);
        k(a,
            d);
    ...
    var i = a,
        ...
        j = [],
        o = "|",
        x = i.chunking.enabled && qq.isFileChunkingSupported(),
        w = i.resume.enabled && x && qq.areCookiesEnabled(),
        u;
    u = null !== i.resume.id && void 0 !== i.resume.id && !qq.isFunction(i.resume.id) && !qq.isObject(i.resume.id) ? i.resume.id : void 0;
    var v = i.forceMultipart || i.paramsInBody,
        m;
    return m = {
        add: function(a) {
            ...
            j[a].uuid = qq.getUniqueId();
            return a
        },
        getName: function(a) {
            a = j[a].file;
            return null !== a.fileName && void 0 !== a.fileName ? a.fileName : a.name
        },
        getSize: function(a) {
            a = j[a].file;
        ...
        upload: function(a, b) {
            ...
            if (x) {
                var d = m.getName(a),
                    c = 0,
                    g, h;
                if (!j[a].remainingChunkIdxs || 0 === j[a].remainingChunkIdxs.length) {
                    j[a].remainingChunkIdxs = [];
                    if (w && !b) {
                        h = qq.getCookie(z(a));
                        var k;
                        h ? (k = h.indexOf(o), g = h.substr(0, k), h = parseInt(h.substr(k + 1, h.length - k), 10), g = {
                            uuid: g,
                            part: h
                        }) : g = void 0;
                        g && (h = e(a, g.part), !1 !== i.onResume(a, d, n(h)) && (c = g.part, j[a].uuid = g.uuid, j[a].loaded = h.start, j[a].attemptingResume = !0, r("Resuming " + d + " at partition index " + c)))
                    }
                    for (d = f(a) - 1; d >= c; d -= 1) j[a].remainingChunkIdxs.unshift(d)
                }
                p(a)
            } else s(a)
        },
        ...
        getResumableFilesData: function() {
            var a = [],
                b = [];
            return x && w ? (a = void 0 === u ? qq.getCookieNames(RegExp("^qqfilechunk\\" + o + ".+\\" + o + "\\d+\\" + o + i.chunking.partSize + "=")) : qq.getCookieNames(RegExp("^qqfilechunk\\" + o + ".+\\" + o + "\\d+\\" + o + i.chunking.partSize + "\\" + o + u + "=")), qq.each(a, function(a, c) {
                var d = c.split(o),
                    e = qq.getCookie(c).split(o);
                b.push({
